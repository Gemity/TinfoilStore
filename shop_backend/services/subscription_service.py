...
def _effective_status(sub: Subscription) -> str:
    ...
    if sub.ends_at.replace(tzinfo=timezone.utc) < now:
        return "expired"
    if sub.starts_at.replace(tzinfo=timezone.utc) <= now:
        return "active"
    return "pending"
...
def grant_subscription(db: Session, user_id: uuid.UUID, days: int, actor_id: uuid.UUID) -> Subscription:
    now = datetime.now(tz=timezone.utc)
    sub = Subscription(
        user_id=user_id,
